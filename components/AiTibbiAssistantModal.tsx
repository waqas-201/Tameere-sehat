...
export default function AiTibbiAssistantModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 animate-in fade-in">
      <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden relative max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="bg-[#0b2317] text-white p-4 sm:p-5 flex items-center justify-between border-b border-emerald-950">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-bold text-amber-300 uppercase tracking-widest block">
                {language === 'ur' ? '24/7 ڈیجیٹل طبی مشیر' : 'AI Unani Tibbi Assistant'}
              </span>
              <h2 className="text-base sm:text-lg font-serif font-bold text-white">
                {language === 'ur' ? 'تعمیرِ صحت ڈیجیٹل طبیب و مشیر' : 'Tameer-e-Sehat Virtual Hakeem AI'}
              </h2>
            </div>
          </div>
        ...
        {/* Quick Prompts Bar */}
        <div className="bg-slate-50 border-b border-slate-200/80 px-4 py-2.5 overflow-x-auto scrollbar-none flex gap-2">
          {quickPrompts.map((prompt, i) => (
            <button
              key={i}
              onClick={() => handleSend(prompt)}
              className="text-[11px] font-bold bg-white hover:bg-[#f0faf4] text-slate-700 hover:text-[#00873E] px-3 py-1.5 rounded-full border border-slate-200 hover:border-[#b0e6c4] transition-all shrink-0 shadow-2xs"
            >
              {prompt}
            </button>
          ))}
        ...
        {/* Chat History */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 max-h-[50vh] bg-[#f8faf9]">
          {messages.map((msg, i) => (
            <div
              key={i}
            ...
            >
              ...
              <div
                className={`p-4 rounded-2xl text-xs sm:text-sm leading-relaxed max-w-[82%] sm:max-w-[75%] ${
                  msg.role === 'user'
                    ? 'bg-[#00873E] text-white rounded-tr-xs shadow-xs'
                    : 'bg-white text-slate-800 border border-slate-200 rounded-tl-xs shadow-2xs whitespace-pre-wrap'
                }`}
              >
                {msg.text}
              </div>
          ...
          {loading && (
            <div className="flex gap-3 items-start">
              <div className="w-8 h-8 rounded-xl bg-[#00873E] text-white flex items-center justify-center">
                <Bot className="w-4 h-4 animate-spin" />
              </div>
              <div className="bg-white border border-slate-200 p-3.5 rounded-2xl rounded-tl-xs text-xs text-slate-500 italic">
                {language === 'ur' ? 'طبی مشیر نباتاتی کتب اور یونانی فارماکوپیا سے رہنمائی تلاش کر رہا ہے...' : 'Analyzing Unani references and botanical pharmacopeia...'}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="p-3 sm:p-4 bg-white border-t border-slate-200 space-y-2">
          ...
          >
            <input
              ...
              className={`flex-1 bg-slate-100 focus:bg-white text-xs sm:text-sm p-3 rounded-xl border border-slate-200 focus:border-[#00873E] focus:ring-2 focus:ring-[#00873E]/20 outline-none transition-all ${
            ...
            <button
              ...
              className="p-3 rounded-xl bg-[#00873E] hover:bg-[#007335] text-white disabled:opacity-40 transition-all shrink-0 shadow-xs"
              aria-label="Send query"
            >
              <Send className={`w-4 h-4 ${language === 'ur' ? 'rotate-180' : ''}`} />
            </button>
          </form>
          ...
          <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-500 pt-1">
            <div className="flex items-center gap-1 text-slate-400">
              ...
              <span>
                {language === 'ur'
                  ? 'پیچیدہ یا پرانے امراض کے لیے ہمیشہ ہمارے مستند طبیب سے براہ راست رابطہ کریں۔'
                  : 'For complex acute conditions, consult our certified Hakeem directly.'}
              </span>
            </div>
            ...
            <a
              ...
              className="text-[#00873E] hover:underline font-bold flex items-center gap-1"
            >
              <MessageSquare className="w-3 h-3 text-[#00873E]" />
              <span>{language === 'ur' ? 'واٹس ایپ پر حکیم صاحب سے رابطہ کریں' : 'Talk to Hakeem on WhatsApp'}</span>
            </a>
          </div>
        </div>
