...
export default function MizajTestPage() {
  ...
  const mizajProfiles = {
    safrawi: {
      ...
      descUr: 'آپ کے جسم میں صفراء (گرمی و خشکی) کا غلبہ ہے۔ آپ کا میٹابولزم تیز ہے لیکن جگر و معدے کی گرمی، جلن اور پیاس کی زیادتی ہو سکتی ہے۔',
      dietEat: ['Arq Kasni', 'Barley Water (Sattoo)', 'Tukhm Balanga', 'Pomegranate', 'Cucumber', 'Melon', 'Coriander'],
      dietAvoid: ['Excess Red Meat', 'Heavily Spiced Curries', 'Fried Pakoras', 'Excessive Garlic', 'Black Pepper'],
    ...
    damwi: {
      ...
      descUr: 'آپ کا مزاج خون کی کثرت اور تری پر مشتمل ہے۔ آپ کی توانائی بہترین ہے، لیکن فشارِ خون (بلڈ پریشر) اور خون کی صفائی کا خیال رکھنا ضروری ہے۔',
      dietEat: ['Kashmiri Zafran Tea', 'Apple', 'Beetroot', 'Ispaghol Husk', 'Fresh Green Salad'],
      dietAvoid: ['Excessive Sweets', 'Very Oily Greasy Foods', 'Overeating at Night'],
    ...
    balghami: {
      ...
      descUr: 'آپ کے جسم میں رطوبت اور ٹھنڈک زیادہ ہے۔ ہاضمہ سست ہو سکتا ہے، لہٰذا گرم خشک اور مقوی ادویات آپ کے لیے مفید ہیں۔',
      dietEat: ['Pure Himalayan Shilajit', 'Ginger Tea', 'Honey', 'Cloves & Cinnamon', 'Dry Fruits (Almonds, Walnuts)'],
      dietAvoid: ['Cold Iced Water', 'Excess Dairy (Cold Milk, Curd)', 'Cucumber in Winter', 'Heavy Rice'],
    ...
    sawdawi: {
      ...
      dietEat: ['Gond Katira Drink', 'Badam Roghan (Almond Oil in warm milk)', 'Mutton Soup', 'Figs (Injeer)', 'Dates'],
      dietAvoid: ['Stale Leftovers', 'Eggplant (Baingan)', 'Lentils (Masoor Daal)', 'Excess Tea & Coffee'],
          ...
          <div className="grid grid-cols-1 gap-3">
            {QUESTIONS[currentStep].options.map((opt, idx) => (
              <button
                ...
                className="p-4 sm:p-5 rounded-2xl border border-stone-200 hover:border-[#199b50] bg-stone-50/60 hover:bg-emerald-50/50 text-left transition-all group flex items-center justify-between"
              >
          ...
          <div className="bg-[#0e2a1f] text-white rounded-3xl p-8 sm:p-10 shadow-xl space-y-6 border border-emerald-900">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-emerald-800/80">
              ...
              <div className="p-4 rounded-2xl bg-white/10 backdrop-blur-xs border border-white/15 space-y-2">
                <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-300 flex items-center gap-1.5">
                  ...
                  <span>Beneficial Foods (Aghziya):</span>
                </h4>
                <ul className="text-xs text-stone-200 space-y-1">
                  {currentProfile.dietEat.map((food, i) => (
                    <li key={i}>• {food}</li>
                  ))}
                </ul>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-white/10 backdrop-blur-xs border border-white/15 space-y-2">
                <h4 className="text-xs font-bold uppercase tracking-wider text-amber-300 flex items-center gap-1.5">
                  ...
                  <span>Foods to Avoid (Parhez):</span>
                </h4>
                <ul className="text-xs text-stone-200 space-y-1">
                  {currentProfile.dietAvoid.map((food, i) => (
                    <li key={i}>• {food}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center gap-3">
              ...
              >
                ...
                <span>Discuss this Result with Hakeem on WhatsApp</span>
              </a>
              ...
              >
                <span>Book Detailed Virtual Clinic</span>
